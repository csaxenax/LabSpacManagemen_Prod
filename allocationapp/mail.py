...
<p text-align:center font-size:8px font-family:Verdana><i>This message auto generated by <i><a href="#https://ilam.intel.com/" target='_blank'>Lab Space Management</a></p>
...
<tr style={benchVisibilty}>
<td>15</td>
<td>Benches</td>
<td>{bench_data}</td>
</tr>

<tr style={rack_shelf_visibility}>
<td>16</td>
<td>Rack-Shelf</td>
<td>{rack_shelf}</td>
</tr>
...
<p font-size:10px font-family:Intel Clear>Hi {User} ( WWID No. {WWID})</p>
<p font-size:10px font-family:Intel Clear>{message} at {LabName}. ( RequestId:{id} )  </p>
<p font-size:10px font-family:Intel Clear>Click here to  <a href="https://ilam.intel.com/allocation?id={id}" target='_blank'>Extend</a></p>
<P>
<table  bordercolor="solid black" border="1"  cellpadding="5" width="800" 
                            style ='border-collapse:collapse;'>
<tr>
...
<body>
<img src="https://sivindicator.intel.com/IMG/LAB.png" alt="LabSpaceManagement" width="800" height="70">
<p text-align:center font-size:8px font-family:Verdana><i>This message auto generated by <i><a href="https://sivindicator.intel.com/workbench/#/home" target='_blank'>Lab Space Management</a></p>

<p font-size:10px font-family:Intel Clear>Hi</p>
<p font-size:10px font-family:Intel Clear>{message})  </p>
<P>
<table  bordercolor="solid black" border="1"  cellpadding="5" width="800" 
                            style ='border-collapse:collapse;'>
...
<tr>
<td>4</td>
<td>Allocated Count</td>
<td>{allocated_data}</td>
</tr>
<tr>
<td>5</td>
<td>All Value</td>
...
class Email:
    ...
    def sendmail(self):
        ...
        try:
            ...
            rack_shelf = ",".join(self.data['Rack-Shelf'])
            benchVisibilty = "display:none;" if not bench_data else ""
            rack_shelf_visibility = "display:none;" if not rack_shelf else ""
            final_message = Bench_html_text.format(LabName=labname, User=User,WWID=WWID,message=message,program_name=program,sku=sku,
                                 lab_name=labname,vendor_name=vendor,allocatedto=allocatedto,department=department,
                                 from_ww=fromww,to_ww=toww,remarks=remarks,duration=duration,number_of_benches=number_of_benches,
                                 bench_data=bench_data,rack_shelf=rack_shelf,team=team,function=function,id=id,notifyto=notifyto,requestedBy=requestedBy,deallocatedby=deallocatedby,rack_shelf_visibility=rack_shelf_visibility,benchVisibilty=benchVisibilty)
...
class CountConfirmation:
    ...
    def sendmail(self):
        ...
        message = self.data['message']
        team = self.data['team']
        allocated_data = self.data['bench_data_length']
        free_bench = self.data['free_report_count']
        total = self.data['total']
        all_value = self.data['all_value']
        matched = self.data['match']

        # Determine if counts match or not
        if matched:
            status_message = "The count matches the expected value."
        else:
            status_message = "The count does NOT match the expected value."

        final_message = count_html_text.format(
            team = team,
            message=message,
    ...
    def sendmail(self):
        ...
        try:
            ...
            final_message = Bench_Expiry_html_text.format(LabName=labname, User=User,WWID=WWID,message=message,program_name=program,sku=sku,
                                 ...
                                 from_ww=fromww,to_ww=toww,remarks=remarks,duration=duration,number_of_benches=number_of_benches,
                                 bench_data=bench_data,rack_shelf=rack_shelf,team=team,department=department,function=function,id=id,notifyto=notifyto
                                 ,rack_shelf_visibility=rack_shelf_visibility,benchVisibilty=benchVisibilty)
            SendEmail(From ,TO, CC, Subject, final_message)
        except Exception as e:
            print(e)
            print(traceback.format_exc())
